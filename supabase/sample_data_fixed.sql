-- Sample Data for Recipe Sharing Platform (FIXED VERSION)
-- This script creates 3 test users and 10 sample recipes
-- Run this in Supabase SQL Editor after setting up your database

-- IMPORTANT: Make sure you have at least 3 users in your system!
-- Create them through the signup page if needed.

-- ============================================
-- UPDATE PROFILES AND INSERT RECIPES
-- ============================================
-- This uses a DO block to safely get user IDs and insert data
...
BEGIN
  ...
  IF user_count < 3 THEN
    RAISE EXCEPTION 'You need at least 3 users in your system. Current count: %. Please create 3 users first through the signup page.', user_count;
  END IF;
  
  -- Get the first 3 user IDs from profiles
  ...
  IF user1_id IS NULL OR user2_id IS NULL OR user3_id IS NULL THEN
    RAISE EXCEPTION 'Could not find 3 valid user IDs. Please ensure you have at least 3 users with profiles.';
  END IF;
  
  RAISE NOTICE 'Using user IDs: %, %, %', user1_id, user2_id, user3_id;
  ...
  RAISE NOTICE 'Updated profiles for 3 users';
  ...
  RAISE NOTICE 'Successfully inserted 10 recipes!';

...
-- ============================================
-- STEP 1: Get User IDs and Update Profiles
-- ============================================
...
BEGIN
  ...
  IF user_count < 3 THEN
    RAISE EXCEPTION 'You need at least 3 users. Currently you have % users. Please create more users through the signup page first.', user_count;
  END IF;
  ...
  -- Verify we got all 3 IDs
  IF user1_id IS NULL OR user2_id IS NULL OR user3_id IS NULL THEN
    RAISE EXCEPTION 'Could not find 3 user IDs. Please ensure you have at least 3 users.';
  END IF;
  ...
  RAISE NOTICE 'Updated profiles for 3 users: %, %, %', user1_id, user2_id, user3_id;
END $$;

-- ============================================
-- STEP 2: Insert 10 Sample Recipes
-- ============================================
...
SELECT 
  ...
  true,
  NOW() - INTERVAL '5 days'
WHERE EXISTS (SELECT 1 FROM profiles ORDER BY created_at LIMIT 1 OFFSET 0);
...
SELECT 
  ...
  true,
  NOW() - INTERVAL '4 days'
WHERE EXISTS (SELECT 1 FROM profiles ORDER BY created_at LIMIT 1 OFFSET 1);
...
SELECT 
  ...
  true,
  NOW() - INTERVAL '3 days'
WHERE EXISTS (SELECT 1 FROM profiles ORDER BY created_at LIMIT 1 OFFSET 2);
